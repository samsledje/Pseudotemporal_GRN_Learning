...
opts_knit$set(root.dir = "/Users/dkaur/Documents/Pseudotemporal_GRN_Learning")
...
sim.obs.from.distribution <- function (layers, N.SAMPLES, distribution="bayesian") {
  ...
  for (i in seq(K.clusters)) {
    ...
    if (distribution == "poisson") {
      ...
      layer.sims <- apply(means, 1, function(x){rnorm(N.SAMPLES,x)})
    }
...
for (i in 1:ITERS){
  if (i %% 10 == 0) {print(i)}
  res <- create_df(reduced.layers, N.SAMPLES, distribution = "poisson")
  bndf <- res$df
  bl <- res$bl
...
```{r "Visualize network"}
plot.adjacency.matrix(adjacency, paste0("img/",cell.type,"_poisson_adjacency.png"))
```
