...
HDmarkers <- as.matrix(cbind(rep(1,length(HD.markers$genes)), rep(1,length(HD.markers$genes))))
colnames(HDmarkers) <- c("Disease.Association", "Nothing")
rownames(HDmarkers) <- HD.markers$genes
HDmarkers <- Matrix(HDmarkers, sparse=TRUE)
enrichment.out <- t(geneset.enrichment.archetype(HD.dat$ACTIONet, ADmarkers))
