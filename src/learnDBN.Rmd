...
sim.obs.from.distribution <- function (layers, N.SAMPLES, distribution="norm") {
  simulated.df <- NULL
...
create_df <- function(layers, sample.size, distribution="norm"){
  bl <- data.frame()
  bndf <- NULL
  prevdf <- NULL
  prevcols <- c()
...
for (i in 1:ITERS){
  if (i %% 10 == 0) {print(i)}
  res <- create_df(reduced.layers, 10000, distribution = "splatter")
  bndf <- res$df
  bl <- res$bl
